...
func _run() -> void:
	...
	for i: int in range(7): game.inventory.add_item(preload("res://data/loot_data.gd").make_weapon("boardwalk", 6, "legendary", 456+i))
	for tab: String in ["inventory", "supplies", "forge", "perks", "dog"]:
		game.open_inventory(tab)
		await _settle()
		_measure(game.ui.shop, "progression " + tab)
	game.open_inventory("inventory")
	await _settle()
	game.ui.shop._open_item_inspector()
	await _settle()
	_measure(game.ui.shop, "item inspector")
	var old_overlay: WeakRef = weakref(game.ui.shop._overlay)
	game.ui.shop.refresh()
	await _settle()
	_check(old_overlay.get_ref() == null and is_instance_valid(game.ui.shop._overlay), "item actions refresh inspector instead of leaving stale stats")
	game.ui.shop._switch_tab("supplies")
	await _settle()
	_check(game.ui.shop._overlay == null, "changing tabs removes inspector")
	game.resume_game()
	...
	var coop_menu: Node = game.get_node("CoopMenu")
	coop_menu.open()
	await _settle()
	_measure(coop_menu.panel,"cooperative modal")
	_check(Rect2(Vector2.ZERO,Vector2(1280,720)).encloses(coop_menu.panel.get_global_rect()),"Cooperative modal remains fully on screen")
	_check(coop_menu.backdrop.visible and coop_menu.backdrop.mouse_filter==Control.MOUSE_FILTER_STOP,"Cooperative modal blocks clicks on the menu behind it")
	coop_menu.close()
	game.ui.hide_menus()
	...
	for key: String in ["round","phase","remaining","health","ammo","ammo_reserve","grenade_count","medkit_count","ammo_supply_count"]:
		var label: Label = game.ui.labels[key]
		var ink := label.get_theme_font("font").get_string_size(label.text,HORIZONTAL_ALIGNMENT_LEFT,-1,label.get_theme_font_size("font_size"))
		_check(label.size.x+0.5 >= ink.x and label.size.y+0.5 >= ink.y,"HUD critical readout retains complete glyphs: "+key)
	for control_name: String in ["RoundReadout","VitalReadout","WeaponReadout"]:
		var content: Control = game.ui.hud.get_node(control_name)
		var background: Panel = game.ui.hud.get_node(control_name+"Background")
		_check(background.get_index()<content.get_index() and not background.is_set_as_top_level(),"HUD backing renders below all its text: "+control_name)
		_check(background.get_theme_stylebox("panel").bg_color.a==1.0,"HUD backing stays opaque: "+control_name)
